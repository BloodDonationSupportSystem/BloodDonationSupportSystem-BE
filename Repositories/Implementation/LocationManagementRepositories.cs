...
namespace Repositories.Implementation
{
    ...
    public class LocationOperatingHoursRepository : GenericRepository<LocationOperatingHours>, ILocationOperatingHoursRepository
    {
        public LocationOperatingHoursRepository(ApplicationDbContext context) : base(context) { }

        public async Task<IEnumerable<LocationOperatingHours>> GetByLocationIdAsync(Guid locationId)
        {
            return await _dbSet
                .Include(loh => loh.Location)
                .Where(loh => loh.LocationId == locationId && loh.DeletedTime == null)
                .OrderBy(loh => loh.DayOfWeek)
                .ToListAsync();
        }

        public async Task<LocationOperatingHours?> GetByLocationAndDayAsync(Guid locationId, DayOfWeek dayOfWeek)
        {
            return await _dbSet
                .FirstOrDefaultAsync(loh => loh.LocationId == locationId && 
                                          loh.DayOfWeek == dayOfWeek && 
                                          loh.DeletedTime == null);
        }

        public async Task<bool> IsLocationOpenAsync(Guid locationId, DayOfWeek dayOfWeek, string timeSlot)
        {
            var operatingHours = await GetByLocationAndDayAsync(locationId, dayOfWeek);
            
            if (operatingHours == null || operatingHours.IsClosed || !operatingHours.IsActive)
                return false;

            return timeSlot.ToLower() switch
            {
                "morning" => operatingHours.MorningStartTime < operatingHours.MorningEndTime,
                "afternoon" => operatingHours.AfternoonStartTime < operatingHours.AfternoonEndTime,
                "evening" => operatingHours.EveningStartTime.HasValue && 
                           operatingHours.EveningEndTime.HasValue &&
                           operatingHours.EveningStartTime < operatingHours.EveningEndTime,
                _ => false
            };
        }

        public async Task<TimeSpan?> GetTimeSlotHoursAsync(Guid locationId, DayOfWeek dayOfWeek, string timeSlot)
        {
            var operatingHours = await GetByLocationAndDayAsync(locationId, dayOfWeek);
            ...
            return timeSlot.ToLower() switch
            {
                "morning" => operatingHours.MorningEndTime - operatingHours.MorningStartTime,
                "afternoon" => operatingHours.AfternoonEndTime - operatingHours.AfternoonStartTime,
                "evening" => operatingHours.EveningEndTime.HasValue && operatingHours.EveningStartTime.HasValue
                           ? operatingHours.EveningEndTime - operatingHours.EveningStartTime
                           : null,
                _ => null
            };
        }
